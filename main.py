...
seed_everything(500) # set random seed
...
trained_wts = torch.Tensor(np.load('fnodes1000.npy'))
# trained_wts=torch.clamp(trained_wts,-20,20)
means = trained_wts.mean(dim=0, keepdim=True)
stds = trained_wts.std(dim=0, keepdim=True)
trained_wts = (trained_wts - means) / (50.0*stds)
# trained_wts = 10.0*normalize(trained_wts, p=2.0, dim = 0)

print(trained_wts.mean())
print("trained wts: " , trained_wts.size())

# Train/val/test split
transform = RandomLinkSplit(is_undirected=True, add_negative_train_samples=False, neg_sampling_ratio=0)#,num_val=0.1, num_test=0.2)
...
pt=train_split.edge_label_index
# print(train_split)
train_split.edge_label_index = pt[:,pt[0,:]<num_playlists]
# print(pt[:,pt[0,:]>num_playlists].size())
print(train_split)
print(val_split)
print(test_split)
...
#         torch.nn.init.normal_(self.embeddings.weight, std=0.1)
        #pretrained wts
        self.embeddings = torch.nn.Embedding.from_pretrained(trained_wts,freeze=False)
...
def test(model, data_mp, loader, k, device, save_dir, epoch):
    ...
    with torch.no_grad():
        ...
        for batch in loader:
            ...
            recalls = model.evaluation(data_mp, batch, 50)
            for playlist_idx in recalls:
                assert playlist_idx not in all_recalls
            all_recalls1.update(recalls)
            
            recalls = model.evaluation(data_mp, batch, 100)
            for playlist_idx in recalls:
                assert playlist_idx not in all_recalls
            all_recalls2.update(recalls)
    ...
    print("recall@50: ",np.mean(list(all_recalls1.values())))
    print("recall@100: ",np.mean(list(all_recalls2.values())))
...
epochs = 10     # number of training epochs (we are keeping it relatively low so that this Colab runs fast)
...
# trained_wts=trained_wts.to(device)
...
opt = torch.optim.Adam(gnn.parameters(), lr=5e-4) # using Adam optimizer
